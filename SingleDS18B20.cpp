...
SingleDS18B20::SingleDS18B20(uint8_t owPin): OneWire(owPin) {
  ...
  if (!readScratchPad(_owROMCode, data)) return;
  ...
  if (data[2] != 0x50 || data[3] != 0x00 || data[4] != 0x5F) {
    ...
    if (!readScratchPad(_owROMCode, data)) return; //if scratchPad read failed then stop
...
float SingleDS18B20::ReadTemp() {
  ...
  if (!readScratchPad(_owROMCode, data)) return 12.3456;
  ...
  if (_owROMCode[0] == 0x10) { //type S temp Sensor
